...
class VideosRepository {
    create(request: Request, response: Response){
        const {user_id, title, description, channel, views} = request.body;
        pool.getConnection((err: any, connection:any) => {
            if (err) {
                return response.status(500).json({ error: 'Problema na conexão' });
            }
            connection.query(
                'INSERT INTO videos (videos_id, user_id, title, description, channel, views, time) VALUES (?,?,?,?,?,?,NOW())',
                [uuidv4(), user_id, title, description, channel, views],
